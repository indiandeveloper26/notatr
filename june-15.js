...
//   const rsi = RSI.calculate({ values: close, period: 14 }).at(-1);
//   const ema = EMA.calculate({ values: close, period: 14 }).at(-1);
//   const sma200 = SMA.calculate({ values: close, period: 200 }).at(-1); // ✅ Add SMA 200
//   const macd = MACD.calculate({ values: close, fastPeriod: 12, slowPeriod: 26, signalPeriod: 9 }).at(-1);
//   const atr = ATR.calculate({ high, low, close, period: 14 }).at(-1);
//   const volumeSMA = SMA.calculate({ values: volume, period: 20 }).at(-1);
...
//   return {
//     signal, entry: lastClose, rsi, ema, sma200, macd, atr, target, stoploss,
//     volume: lastVolume, volumeSMA
//   };
// }
...
// console.log("✅ Signal Bot with SMA 200 Running...");
...
// setInterval(checkOneSymbolPerMinute, 2 * 60 * 1000); // every 2 minutes
// console.log("✅ SMA 200 Based Signal Bot Running...");
...
function analyze(candles) {
  ...
  // Time Filter (Avoid 2 AM to 6 AM UTC)
